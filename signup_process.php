<?php
	session_start();
	include("db.php");
	//This will deactivate the automatic error messaging, so this allow for manual error handling. 
	...
	echo "<body>";
		...
		//if any of the mandotory fields are empty
		if (empty($fname) or empty($lname) or empty($address) or empty($postcode) or empty($telno) or empty($email)or 
		empty($password)or empty($cpassword)) //check if any of the variables that captured the posted values are empty
		{
